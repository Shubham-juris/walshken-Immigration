...
const PermanentResidency = () => {
  ...
  // 1. Country-wise PR Programs (with brightened, recognizable background images)
  const countryPRPrograms = [
    {
      country: "Canada",
      programs: [
  ...
  return (
    <div className="bg-gradient-to-r from-black via-gray-900 to-red-950 text-white min-h-screen font-sans">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          >
            <div className="bg-gradient-to-br from-gray-950 to-gray-900 p-8 rounded-2xl border border-red-800/20">
              ...
              <p className="text-gray-300 leading-relaxed text-sm">
                At <span className="text-white font-bold">Walshken Immigration</span>, we closely inspect every document and construct responsive profiles to mitigate these risks. If your profile encounters obstacles like procedural fairness reviews, we assist you in drafting transparent, policy-aligned responses.
              </p>
            </div>
          </motion.div>
      ...
      {/* ────────────────────────────────────────────────────────
          4. COUNTRY-WISE PR PROGRAMS (BRIGHTENED BACKGROUNDS + RECOGNIZABLE LANDMARKS)
          ──────────────────────────────────────────────────────── */}
      <section className="py-20 px-6 max-w-7xl mx-auto border-t border-red-950/40">
        <div className="max-w-6xl mx-auto mb-16 text-center">
          <p className="text-red-500 font-semibold uppercase tracking-widest text-xs mb-2">Global Programs</p>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {countryPRPrograms.map((country, index) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
            >
              {/* Brightened, Clearly Recognizable Background Image */}
              <img
                src={country.bgImg}
                alt={country.country}
                className="absolute inset-0 w-full h-full object-cover filter brightness-110 opacity-55 group-hover:opacity-70 group-hover:scale-105 transition-all duration-500"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/80 to-black/45"></div>

              {/* Card Content with Crisp Legibility */}
              <div className="relative z-10">
                <div className="flex items-center mb-5 pb-3 border-b border-red-500/30">
                  <span className="text-3xl mr-3 drop-shadow">{country.flag}</span>
                  <h3 className="text-2xl font-extrabold text-red-400 drop-shadow-md">
                    {country.country}
                  </h3>
                </div>
                
                <div className="mb-4">
                  <h4 className="text-white font-bold text-xs uppercase tracking-wider mb-2.5 drop-shadow">Programs Handled:</h4>
                  <ul className="text-gray-200 text-xs space-y-1.5 font-medium">
                    {country.programs.map((program, i) => (
                      <li key={i} className="flex items-start drop-shadow-sm">
                        <span className="text-red-400 mr-2 font-bold">•</span> {program}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="relative z-10 mt-5 pt-4 border-t border-red-950/60 space-y-2.5">
                <div>
                  <h4 className="text-white font-bold text-xs drop-shadow">Path Benefits:</h4>
                  <p className="text-gray-300 text-xs leading-relaxed font-medium drop-shadow-sm">{country.benefits}</p>
                </div>
                
                <div>
                  <h4 className="text-white font-bold text-xs drop-shadow">Estimated Processing Time:</h4>
                  <p className="text-red-300 text-xs font-bold drop-shadow">{country.processingTime}</p>
                </div>
              </div>
            </motion.div>
          ))}
